...
#ifdef CUBOX_CONSUMER_SIZE
  auto& ctl = CameraController::get(m_cameraView, CUBOX_RPODUCER_SIZE, CUBOX_CONSUMER_SIZE);
#else
...
#ifdef __CUBOX__
  // [cubox] FPS min=3, max=90
  cout << "[cubox] FPS min=" << m_fpsMin << ", max=" << floor(fpsMinMax.second) << endl;
#endif
...
#ifdef CUBOX_DEFAULT_CAM_PROP_8BIT
  m_8bit.set_active();
#endif
...
#ifdef CUBOX_CONSUMER_SIZE
    ctl.startProducer(CUBOX_RPODUCER_SIZE);
    ctl.startConsumers(CUBOX_CONSUMER_SIZE);
#else
    ctl.startProducer(1);
...
#ifdef CUBOX_CONSUMER_SIZE
    vector<string> dirnames(CUBOX_CONSUMER_SIZE);
#else
    vector<string> dirnames(2);
#endif
    for (auto k = 0; k < dirnames.size(); ++k) {
#ifdef __CUBOX__
      // dirnames[k] = "/home/cubox/cubox/Surround360/surround360_camera_ctl_ui"; // cubox test
      dirnames[k] = "/home/cubox/Desktop/test"; // cubox test for run_all.py
      cout << "[cubox] MainWindow::takeNameDialog() force set dirnames[" << k << "] "
        << dirnames[k] << endl;
#else
...
#ifdef __CUBOX__
  string dirname = "/home/cubox/Desktop/test";
  cout << "[cubox] MainWindow::singleTakeDialog() force set dirname=" << dirname << endl;
...
#ifdef CUBOX_DEFAULT_CAM_PROP_FPS
  newFps = CUBOX_DEFAULT_CAM_PROP_FPS;
#endif
