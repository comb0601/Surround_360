...
#if 0 // __CUBOX__
  cout << "[cubox] PointGreyCamera::getCamera(" << index
    << ") fmt7Info.maxWidth=" << fmt7Info.maxWidth
    << ", fmt7Info.maxHeight=" << fmt7Info.maxHeight << endl;
#endif
...
#if 0 // __CUBOX__
  cerr << "\tcubox before m_camera->RetrieveBuffer() +++" << endl;
  Error error = m_camera->RetrieveBuffer(img);
  cerr << "\tcubox after m_camera->RetrieveBuffer() --> error=" << error.GetType() << endl;
  // error=32 (PGRERROR_ISOCH_NOT_STARTED) ==> Isochronous transfer has not been started
  // cat /sys/module/usbcore/parameters/usbfs_memory_mb --> 8000
  // sudo sh -c 'echo 10240 > /sys/module/usbcore/parameters/usbfs_memory_mb'
#else
  Error error = m_camera->RetrieveBuffer(img);
#endif
