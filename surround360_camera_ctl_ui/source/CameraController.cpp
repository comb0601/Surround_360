...
#ifdef __CUBOX__
  cout << "[cubox] CameraController() m_width=" << m_width << ", m_height=" << m_height << endl;
#endif
}
...
#ifdef __CUBOX__
  // Fix single capture release mode runtime error(Bad file descriptor)
  return true;
#endif
}
...
void CameraController::startProducer(const unsigned int count) {
  ...
  for (int pid = 0; pid < count; ++pid) {
    ...
    //m_prodThread.back().detach(); // cubox added to fix but not worked!
  }
}
...
void CameraController::cameraProducer(const uint32_t id) {
  // cubox: only single producer work
  // const size_t camerasPerProducer = (m_camera.size() + m_producerCount - 1) / m_producerCount;
...
// #define CUBOX_DEBUG_LOG_FRAMECOUNTER
#ifdef CUBOX_DEBUG_LOG_FRAMECOUNTER
    static int __cubox_all_cam_loop = 0;
    __cubox_all_cam_loop++;
#endif
    ...
    for (auto i = cameraOffset; i < lastCamera; ++i) {
      ...
      try {
        ...
        frameCounter[i] = frame[i].GetMetadata().embeddedFrameCounter; // cubox: different between cameras
...
#ifdef CUBOX_DEBUG_LOG_FRAMECOUNTER
        if (__cubox_all_cam_loop % 1 == 0) {
          // e.g.
          // [cubox] frameCounter[0] --> 44282, ts=2622 493988
          // [cubox] frameCounter[0] --> 44283, ts=2623 587028
          cout << "[cubox] frameCounter[" << i << "] --> " << frameCounter[i] << ", ts=" << frame[i].GetMetadata().embeddedTimeStamp << endl;
        }
#endif
...
#ifdef CUBOX_CHECK_PROD_cameraSerial_OVERWRITE
          if (nextFrame->cameraSerial < 1000) {
            assert(false);
            while (true) {
              cerr << "[cubox] ERROR: invalid nextFrame->cameraSerial=" << nextFrame->cameraSerial << endl;
              cerr << "[cubox] ERROR: nextFrame->frameNumber=" << nextFrame->frameNumber << endl;
            }
          }
#endif
...
#ifdef __CUBOX__
      // cubox: remove existing .bin files, WARNING O_DIRECT is slow on some system?!(e.g, X570 server)
      fd = open(fname.c_str(), O_WRONLY | O_NONBLOCK | O_CREAT | O_DIRECT | O_TRUNC, 0644);
#else
...
#ifdef CUBOX_CHECK_CONS_WRITE_TIME
        ssize_t count = 0;
        static int write_cnt = 0;
        if (write_cnt++ % 100 == 0) {
          // RTX8000 2~3cam/30fps/12b -> 1~2ms
          CUBOX_TIME_START(s1)
          count = write(fd, next->imageBytes, next->frameSize);
          CUBOX_TIME_PRINT("write", s1);
        } else {
          count = write(fd, next->imageBytes, next->frameSize);
        }
#else
