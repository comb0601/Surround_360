...
if __name__ == "__main__":
  ...
  # __CUBOX__: Fix can't have unbuffered text I/O error, removed 0 param
  # file_runtimes = open(out_dir + "/runtimes.txt", 'w', 0)
  file_runtimes = open(out_dir + "/runtimes.txt", 'w')
  start_time = timer()
  ...
  if black_level_adjust:
    ...
    for i in range(len(out_dirs)):
      ...
      black_level = json.loads(open(out_dirs[i] + "/black_level.txt").read()) # cubox fix error: "!
